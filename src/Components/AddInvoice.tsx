...
export function AddInvoice({ handleClose, setInvoicePageUpdate }: props) {
	...
	function handleInvoiceSaveRequest(data: any) {
		...
		if (isAmountValid && isDescriptionValid) {
			...
			writeIntoDataBase(path, { ...data, id, allocation: 0 });
			reset();
	...
	return (
		<Paper sx={{ p: 2, m: 1 }}>
			...
			{inputValid.amount && <Typography color="red">
				enter correct amount
			</Typography>}
			<Box sx={{ mt: 2, mb: 2 }}>
				<Typography> Invoice description</Typography>
				{inputValid.description && <Typography color="red">
			...
			<Button variant="contained" sx={{ mr: 2 }} onClick={handleSubmit(handleInvoiceSaveRequest)}>Add invoice</Button>
