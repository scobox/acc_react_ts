...
export default function InvoiceTable({ invoiceList: rows, setRerender }: InvoiceTableProps) {
  ...
  const handleAllocationChange = (event: any, index: number) => {
    ...
    updateDataInFirebase(`invoices/2022-2023/${invoiceId}/allocation`, allocationId || 0)
      .then(res => {
                      ...
                      <TableCell align="right">
                        ...
                        <FormControl sx={{ ml: 2, width: 280 }} size="small">
                          ...
                          <Select
                            ...
                            value={allocations[Number(row.allocation)]?.name || "unallocated"}
                            name={row.id.toString()}
                            onChange={(event) => handleAllocationChange(event, index)}
                            label="Allocation"
                          >
                            {Object.keys(allocations).map(menuItem => <MenuItem id={menuItem} key={menuItem} value={allocations[Number(menuItem)]?.name || 0}>{allocations[Number(menuItem)]?.name || "undefined"}</MenuItem>)}
                          </Select>
                        </FormControl >
                      </TableCell>
                    </TableRow>
